...
interface Song {
  ...
  url: string;
  note: string;
  color: string; // Añadimos un color representativo por canción
}
...
const PLAYLIST: Song[] = [
  {
    ...
    color: "#6E00B3" // Rosa neón
  },
  {
    id: 2,
    title: "Buenos Aires en Llamas",
    artist: "Trueno",
    cover: "/fotos/trueno1.jpg",
...
const CardsProject: React.FC = () => {
  ...
  // Efecto para cambiar el color de la página
  useEffect(() => {
    if (selectedSong) {
      document.documentElement.style.setProperty('--accent-color', selectedSong.color);
    } else {
      document.documentElement.style.setProperty('--accent-color', '#6c5ce7'); // Color base
    }
  }, [selectedSong]);
      ...
      <div className="cards-grid">
        {PLAYLIST.map((song) => (
          ...
          >
            ...
            {/* Pequeño indicador de color en la carta */}
            <div style={{ 
              height: '4px', 
              width: '40px', 
        ...
        {selectedSong && (
          ...
          >
            ...
            >
              ...
              <audio src={selectedSong.url} controls autoPlay className="audio-player" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
